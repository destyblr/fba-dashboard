...
exports.handler = async () => {
  ...
  try {
    const res = await fetch('https://www.mediamarkt.de/de/campaign/angebote', {
      headers: HEADERS,
      timeout: 10000,
    });
    ...
    // MediaMarkt uses product card components; selectors may change over time.
    // Note: if the page is fully client-side rendered (React/Next.js hydration),
    // cheerio will see an empty shell and this will return [].
    $('[data-test="product-card"], [class*="ProductCard"], [class*="product-card"]').each((_, el) => {
      ...
      deals.push({
        ...
        link: link.startsWith('http') ? link : link ? `https://www.mediamarkt.de${link}` : null,
        image: image.startsWith('http') ? image : image ? `https://www.mediamarkt.de${image}` : null,
        merchant: 'MediaMarkt',
    ...
    console.error('[deals-mediamarkt] Scrape failed:', err.message);
  }
